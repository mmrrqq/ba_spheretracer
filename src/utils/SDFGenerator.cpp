      ...
      BoxSize(boxSize)
{
    ...
    float scaleFactor = BoxSize / maxMeshDim - 0.1; // ten percent border
    ...
    for (auto v : mesh.vertices())
    {
        ...
        vertices_.at(v.idx()) = Vertex(
            (vertex[0] - barycenter.x) * scaleFactor + BoxSize / 2.0,
            (vertex[1] - barycenter.y) * scaleFactor + BoxSize / 2.0,
            (vertex[2] - barycenter.z) * scaleFactor + BoxSize / 2.0);
    }
...
void SDFGenerator::Dispatch()
{
    ...
    glDispatchCompute(BoxSize / 16, BoxSize / 16, BoxSize / 4);
